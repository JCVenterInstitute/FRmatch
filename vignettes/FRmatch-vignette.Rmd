---
title: "FR-Match: A cluter-to-cluster cell type matching method for single cell RNA-seq experiments"
author: 
...
- Reference dataset
    ...
    + Cell type cluster labels for each cell
...
To install from GitHub repository, please use
```{r, install, eval=FALSE}
install.packages("devtools")
devtools::install_github("JCVenterInstitute/FRmatch")
```

After successful installation, please load FR-Match and other useful pacakges to your R environment
...
We complement this package with a Shiny App, which can be launched by
...
For FR-Match data object, the following data items are essential:
...
In addition, metadata (`@metadata`) such as F-meansure and cluster order are not essential for the core matching algorithm, but these information will facilitate visualization and other customized analysis tools provided in this package.

## An example data object

In this package, we include an example data object `sce.example`. More details please see `help("sce.example")`. A quick look of the data object.
...
Now, look at the cluster sizes in these two datasets.
```{r, clusterSize, fig.height = 9, fig.width = 7}
plot_clusterSize(sce.sam1, sce.sam2)
```

## Bracod plot with marker genes
...
```{r, barcod-plot, fig.height = 5, fig.width = 7}
...
From the adjusted p-value plot, we see that the significance of the match of i6 and i9 clusters is very marginal. We may obtain the actual values from these plots using the `return.value = TRUE` in corresponding plotting functions.
...
We also implemented our own function for Friedman-Rafsky (FR) test ([1979](https://projecteuclid.org/download/pdf_1/euclid.aos/1176344722)) with customized options in `FR.test()`. FR test is a multivariate generalization of non-parametric two-sample test. It is a graphical model based on the concept of the [minimum spanning tree](https://en.wikipedia.org/wiki/Minimum_spanning_tree) (MST). This natually provides a way to visualize high-dimensional clustered data in a low-dimensional plot. Below is a minimal working example with an MST plot.
